...
const deviceWidth = Dimensions.get("window").width;
  ...
  const dynamicPaddingLeft1 = deviceWidth * 0.09;
  ...
  const BottomTabs = () => {
    ...
    return (
      ...
      <View style={styles.inARow}>
        ...
        <TouchableOpacity  activeOpacity={0.4} style={styles.bottomTab} onPress={() => handleTabPress("ProfileSetupPatient")}>
        <View style={styles.inARow}>
...
const Profile1 = () => {
  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      try {
        ...
        const userDoc = await getDoc(doc(FIREBASE_DB, "usersInfo", userId));
        if (userDoc.exists) {
  ...
  const info = [
    { name: "Gender", specialty: userInfo?.gender || "" },
    { name: "Age", specialty: userInfo?.age || "" },
    { name: "Weight", specialty: `${userInfo?.bodyWeight || ""} lb` },
  ];
  const info1 = [
    { name: "Blood Group", specialty: userInfo?.bloodGroup || "" },
    { name: "Health Condition", specialty: userInfo?.medicalHistory || "" },
  ];
  const renderInfo = ({ item }) => (
    ...
    {item.name === "Gender" && <Ionicons name="ios-female" size={30} color="purple" />}
        ...
        {item.name === "Weight" && <Ionicons name="ios-barbell" size={30} color="purple" />}
        {item.name === "Blood Group" && <Ionicons name="ios-heart" size={30} color="purple" />}
        {item.name === "Health Condition" && <Ionicons name="ios-medkit" size={30} color="purple" />}
  ...
  const renderInfo1 = ({ item }) => (
    ...
    {item.name === "Gender" && <Ionicons name="ios-female" size={30} color="purple" />}
        {item.name === "Age" && <Ionicons name="ios-calendar" size={30} color="purple" />}
        {item.name === "Weight" && <Ionicons name="ios-barbell" size={30} color="purple" />}
        {item.name === "Blood Group" && <Ionicons name="ios-water" size={30} color="purple" />}
        {item.name === "Health Condition" && <Ionicons name="ios-body" size={30} color="purple" />}
...
const styles = StyleSheet.create({
  ...
  ameliaRenata: {
    width: 200,
    top: deviceHeight / 2- 265, // Center vertically
