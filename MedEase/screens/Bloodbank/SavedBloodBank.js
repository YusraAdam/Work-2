...
const SavedBloodBank = () => {
  ...
  const fetchFavoriteDrugs = async () => {
    try {
      ...
      if (favDrugsDoc.exists()) {
        ...
        setBloodBankData(favDrugsArray);
      }
  ...
  const removeFromFavorites = async (index, bbId, bbName, bbLocation, bbLogo) => {
    try {
      const user = FIREBASE_AUTH.currentUser;
      const userId = user.uid;
      ...
      setBloodBankData((prevDrugsData) =>
        prevDrugsData.filter((drug) => drug.id !== bbId)
      );
    } catch (error) {
          ...
          {row.map((drug, index) => (
            ...
            >
              <Image
                ...
                contentFit="cover"
                source={{ uri: drug.logo}}
              />
              <View style={styles.pharmacyDetails}>
                <Text style={styles.pharmacyName}>{drug.name}</Text>
                <Text style={styles.pharmacyLocation}>{drug.location}</Text>
                <TouchableOpacity onPress={() =>setSelectedBloodBankId(drug.id)}>
                <Text style={styles.drugType1}>View Available Blood Groups</Text>
                </TouchableOpacity>
                <TouchableOpacity
                style={styles.heartIconContainer}
                onPress={() => removeFromFavorites(index, drug.id, drug.name, drug.location, drug.logo)}
              >
                <Image
                  style={[
