...
const ambulanceData = [
  ...
  { id: 5, name: 'Al-Khidmat', image: require('../../assets/newal.png') , number: 'Helpline: 1030'},
  { id: 6, name: 'Edhi', image: require('../../assets/newedhi.png') , number: 'Helpline: 115'},
];

const SeeAllHelpline= ({ navigation }) => {
  const [isMenuVisible, setIsMenuVisible] = useState(false);
  const deviceWidth = Dimensions.get("window").width;
  const deviceHeight = Dimensions.get("window").height;
  const dynamicPaddingTop = deviceHeight * 0.0;
  const dynamicWidth = deviceWidth * 0.85;
  const dynamicWidth1 = deviceWidth * 0.75;
