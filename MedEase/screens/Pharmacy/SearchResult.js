...
import { StyleSheet, View, Text, Dimensions, Image, FlatList, ActivityIndicator, TouchableOpacity } from "react-native";
...
const SearchResult = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      const { searchText, pharmacyData: pharmIdArray } = route.params || {};
      setSearchText(searchText || "");

      if (pharmIdArray && Array.isArray(pharmIdArray)) {
        // console.log("Fetching data for pharmacyIds:", pharmIdArray);

        const firestore = getFirestore();
        const pharmacyDataPromises = pharmIdArray.map(async (pharmId) => {
          const pharmacyDocRef = doc(firestore, "allPharmacies", pharmId.id);

          try {
            const pharmacyDocSnapshot = await getDoc(pharmacyDocRef);
            const detailedPharmacyInfo = pharmacyDocSnapshot.data();
            ...
            return {
              id: pharmId.id,
              name: detailedPharmacyInfo.name,
              location: detailedPharmacyInfo.location,
              time: detailedPharmacyInfo.time,
              logo: detailedPharmacyInfo.logo,
        ...
        setPharmacyData(filteredPharmacyData);
      }
      setIsLoading(false);
    };
    ...
    <>
      ...
      >
        ...
        >
          ...
          <View style={styles.title}>
            <Text style={[styles.findANear, styles.dentalTypo]}>
              {searchText
                ? `Available Pharmacies For ${searchText} `
                : ""}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.pharmacyDetails}>
          ...
          <TouchableOpacity onPress={() =>navigateToPharmDrugs(pharmacy.id)}>
