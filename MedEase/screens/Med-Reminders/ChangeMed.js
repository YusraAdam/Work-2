...
import { View, Text, TextInput, TouchableOpacity, Switch, StyleSheet, Modal, FlatList, ActivityIndicator} from 'react-native';
...
import { collection, getDocs, query, where, getDoc, doc, addDoc, setDoc} from "@firebase/firestore";
...
const ChangeMed = () => {
    ...
    const fetchMedicineDetails = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          // Set the state variables with the fetched medicine details
          setMedicineName(data.name);
          setMedicineType(data.type);
          setPillType(data.type); // Set pill type
          setDuration(data.duration.toString()); // Set duration as string
          setQuantity(data.quantity.toString());
          setStock(data.availableStock.toString());
          // Set other state variables with the fetched medicine details
  
          // Fetch reminder times and set the state
          if (data.reminderTimes) {
            setReminderTimes(data.reminderTimes);
            setSelectedTimes(data.reminderTimes); // Select default reminder times
          }
  
          // Fetch selected days and set the state
  ...
  }, [id]); // Fetch medicine details when the id changes
  
  useEffect(() => {
    const fetchMedicineFinishAlarm = async () => {
      try {
        // Fetch the medicine finish alarm state from AsyncStorage
        const finishAlarmState = await AsyncStorage.getItem('medicineFinishAlarm');
        if (finishAlarmState !== null) {
          // If the state exists in AsyncStorage, set the state accordingly
          setIsMedicineFinishAlarmOn(finishAlarmState === 'true');
        }
      } catch (error) {
        console.error('Error fetching medicine finish alarm state: ', error);
      }
    };
  
    fetchMedicineFinishAlarm();
  }, []);

  const toggleMedicineFinishAlarm = async () => {
    try {
      const newState = !isMedicineFinishAlarmOn;
      setIsMedicineFinishAlarmOn(newState);
  
      // Update the medicine finish alarm state in AsyncStorage
      await AsyncStorage.setItem('medicineFinishAlarm', newState.toString());
    } catch (error) {
      console.error('Error toggling medicine finish alarm: ', error);
    }
  };
  ...
  const saveReminder = async () => {
    ...
    try {
      ...
      const docRef = doc(FIREBASE_DB, 'medReminder', id); // Reference to the existing document
    await setDoc(docRef, medicineData, { merge: true }); // Merge with existing document
    // console.log('Medicine reminder edited with ID: ', id);
  ...
  return (
    ...
    >
    {isLoading ? ( // Show activity loader if isLoading is true
      <View style={styles.loaderContainer}>
        <ActivityIndicator size="large" color="#a2429e" />
      </View>
    ) : (
    ...
    <View style={[styles.topBar, styles.topBarPosition]}>
            ...
            <SelectDropdown
              ...
              defaultValue={medicineType} // Set default value to the fetched pill type
          ...
          <TouchableOpacity onPress={showTimePicker}>
              <View style={styles.addTimeContainer}>
                ...
                <Text style={styles.addTimeLabel}>Edit Reminder Time </Text>
...
{/* <View style={styles.switchContainer}>
      ...
      {/* <View style={styles.labelContainer}>
        ...
        <Switch
          ...
          onValueChange={toggleMedicineFinishAlarm}
          thumbColor={isMedicineFinishAlarmOn ? 'purple' : '#f4f3f4'}
          trackColor={{ false: 'white', true: '#a2429e' }}
        />
      </View>
      </>
          )}
